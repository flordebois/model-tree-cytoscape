...
def beeswarm(all_contributions, X, y_hat, n_max=5, fig_size=(10, 5),
    ...
    y_hat : array-like, shape (n_samples,)
        Predicted values for VizTree.X_train
    n_max : int, default=5
    ...
    num_nonzero_features = np.sum(np.sum(np.abs(all_contributions), axis=0) > 1e-14)
    ...
    for i in range(num_features_to_plot):
        contribution_i = contributions[:, i]
        ...
        n_bins = 50
        bins = np.linspace(contribution_i.min(), contribution_i.max(), n_bins)
        bin_idx = np.digitize(contribution_i, bins)
        x_pos = np.zeros(len(contribution_i))
        for b in np.unique(bin_idx):
            mask = bin_idx == b
            count = mask.sum()
            offsets = np.linspace(-count // 2, count // 2, count) * 0.07
            x_pos[mask] = offsets

        median = np.nanmedian(feature_values)
        mad = 1.4826 * np.nanmedian(np.abs(feature_values - median))
        vmin = median - 3 * mad
        vmax = median + 3 * mad

        feature_axes[i].scatter(x_pos,
                                contribution_i,
                                c=feature_values.clip(vmin,vmax),
        ...
        feature_axes[i].set_xticks([])
        feature_axes[i].set_ylim([np.min(contribution_i), np.max(contribution_i)])
        ...
        feature_axes[i].spines[['top', 'right', 'bottom', 'left']].set_visible(False)
